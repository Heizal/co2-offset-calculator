...
describe("useEmission Hook", () => {
    test("should fetch emissions successfully", async () => {
        ...
        estimateEmissions.mockResolvedValue({ co2e: 120, co2e_unit: "kg" });
    ...
    test("should handle API errors", async () => {
        // Mock API failure
        estimateEmissions.mockResolvedValue(null);
        const { result } = renderHook(() => useEmission());
        const startDate = new Date('2023-01-01');
        const endDate = new Date('2023-01-31');
        await act(async () => {
    ...
    test("should set loading state during API call", async () => {
        estimateEmissions.mockResolvedValue({ co2e: 150, co2e_unit: "kg" });
        const { result } = renderHook(() => useEmission());
        const startDate = new Date('2023-01-01');
        const endDate = new Date('2023-01-31');
        act(() => {
